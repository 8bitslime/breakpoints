...
module.exports = {
	config: {
		'file_name': {
			'description': "Where to store the generated breakpoint script (requires restart).",
			'type': "string",
			'default': ".breakpoints"
		}
	},
	...
	activate: function() {
		...
		this.pointFilePath = projectPath + '\\' + atom.config.get("Breakpoints.file_name");
		this.pointFilePath = path.normalize(this.pointFilePath);
		
		atom.project.onDidChangePaths(function (newPath) {
			this.pointFilePath = newPath + '\\' + atom.config.get("Breakpoints.file_name");
			this.pointFilePath = path.normalize(this.pointFilePath);
			this.beginPointFile();
		}.bind(this));
